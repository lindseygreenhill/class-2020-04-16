...
logistic_mod <- logistic_reg() %>%
  set_engine("glm")
logistic_fit <- fit(logistic_mod, factor(vote) ~ ideology + income + gender + race, data = nes)
logistic_fit %>% tidy(conf.int=T)

# coef for income: for every increase in income category, with all other
# variables held constant, on average, the probability of voting democrat
# decreases by about .05

# dear boss, people in one income category above another are about .05% less
# likely to vote democrat, and we are pretty sure that number falls within .037%
# and .067%
```
...
vote_tree <- fit(tree_mod,
                 factor(vote) ~ ideology + income + gender + race, data = nes)
vote_tree$fit %>% prp(extra = 6, varlen = 0, faclen=0)
...
# OOB error: 25.33% - it will sample with replacement from our data and then take the average of all of those trees. The prediction error. How wrong can this be. 
...
predictions <- nes %>%
  mutate(modal = 1,
         logistic = predict(logistic_fit, new_data = nes) %>% 
           pull(.pred_class),
         tree = predict(nes_region_tree, new_data = nes) %>% 
           pull(.pred_class),
         forest = predict(nes_forest, new_data = nes) %>% 
           pull(.pred_class))

tibble(modal = mean(predictions$modal == predictions$vote),
       linear = mean(predictions$logistic == predictions$vote),
       tree = mean(predictions$tree == predictions$vote),
       forest = mean(predictions$forest == predictions$vote))
